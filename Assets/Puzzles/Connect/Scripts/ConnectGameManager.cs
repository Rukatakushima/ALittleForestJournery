...
namespace Connect
{
    public class GameManager : BaseGameManager<LevelSpawner, DefaultCameraController, WinConditionChecker, LevelData>
    {
        ...
        {
            cameraController.SetupCamera(currentLevelSize);

            // levelSpawner.Initialize(_nodeGrid, currentLevelSize);
            levelSpawner.SpawnLevel();

            winConditionChecker.Initialize(_nodes);
        }

        public void SetNodes(List<Node> _nodes, Dictionary<Vector2Int, Node> _nodeGrid, int currentLevelSize)
        {
            this._nodes = _nodes;
        ...
        private int currentLevelSize = 5;
